...
from model.utils import trunc_normal_, build_mlp, load_pretrained_weights
...
class VisionTransformerClassifier(nn.Module):
    def __init__(self, arch_name='vit_small', n_label=10, patch_size=16, dropout=0.2, emb_size=768,
                 n_last_blocks=4,
        ...
        if pretrained:
            # self.vit_model.eval()
            load_pretrained_weights(self.vit_model, pretrained_weights, checkpoint_key, arch_name, patch_size)
        ...
        input_size = self.vit_model.embed_dim * (n_last_blocks + int(avgpool_patchtokens))
        ...
        self.classifier = LinearClassifier(self.embedding_size, num_labels=n_label)
    ...
    def fine_tune(self, fine_tune_layers):
        ...
        if fine_tune_layers > 0:
            for p in list(self.vit_model.children())[3].parameters():  # updating LayerNorm
                p.requires_grad = True

            # Last transformer layers to be fine-tuned
            trans_layers = list(list(self.vit_model.children())[2].children())
            for c in trans_layers[len(trans_layers) - fine_tune_layers:]:
                for l in list(c.children()):
                    if type(l) == torch.nn.LayerNorm:
